...
from sqlalchemy import (
    ...
    Text, ForeignKey, Date, LargeBinary,
)
...
# ── Python enums (used in schemas / serialisation only) ──────────────────────
...
# ── Native PostgreSQL ENUM types (create_type=False = already exist in DB) ───
_user_role        = PGENUM('admin', 'manager', 'cashier', 'warehouse',
                            name='user_role', create_type=False)
_payment_method   = PGENUM('cash', 'qr_promptpay', 'bank_transfer', 'credit',
                            name='payment_method', create_type=False)
_payment_status   = PGENUM('pending', 'confirmed', 'failed', 'refunded',
                            name='payment_status', create_type=False)
_order_status     = PGENUM('draft', 'pending', 'completed', 'cancelled', 'refunded',
                            name='order_status', create_type=False)
_stock_tx_type    = PGENUM('receive', 'sale', 'adjustment', 'return', 'damage',
                            name='stock_transaction_type', create_type=False)
_credit_status    = PGENUM('active', 'overdue', 'paid', 'suspended',
                            name='credit_status', create_type=False)
_unit_type        = PGENUM('kg', 'g', 'l', 'ml', 'bag', 'box', 'piece', 'set', 'bottle', 'pack',
...
class User(Base):
    ...
    role            = Column(_user_role, nullable=False, default='cashier')
...
class StockTransaction(Base):
    ...
    id               = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    transaction_type = Column(_stock_tx_type, nullable=False)
...
class Customer(Base):
    ...
    credit_days     = Column(Integer, default=30)
    credit_status   = Column(_credit_status, default='active')
...
class SalesOrder(Base):
    ...
    status           = Column(_order_status, default='draft')
    ...
    payment_method   = Column(_payment_method)
    payment_status   = Column(_payment_status, default='pending')
...
class PaymentTransaction(Base):
    ...
    payment_method  = Column(_payment_method, nullable=False)
    amount          = Column(Numeric(12, 2), nullable=False)
    status          = Column(_payment_status, default='pending')
